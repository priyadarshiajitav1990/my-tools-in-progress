"""
Enterprise-grade script to convert Apigee SpikeArrest/Quota policy to Kong rate-limiting plugin YAML using Jinja template and policy2plugin.json mapping.
- Modular functions, clear comments, mandatory/optional fields.
- Usage: python apigee-policy2rate-limiting.py <policy.xml> <service_name> <output.yaml>
"""
import sys
import os
import json
...
def parse_spikearrest_quota_policy(xml_path):
    """
    Parse Apigee SpikeArrest/Quota XML and extract config for rate-limiting plugin.
    Args:
    ...
    if rate_elem is not None and rate_elem.text:
        import re
        match = re.match(r'(\d+)([a-z]+)', rate_elem.text.strip())
        if match:
...
def main():
    ...
    """
    if len(sys.argv) != 4:
        print("Usage: python apigee-policy2rate-limiting.py <policy.xml> <service_name> <output.yaml>")
        sys.exit(1)
    xml_path = sys.argv[1]
    ...
    kong_plugin = mapper.get(apigee_policy_type, 'rate-limiting')
