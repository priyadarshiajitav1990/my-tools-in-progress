"""
Enterprise_grade script to convert Apigee AccessControl policy to Kong ip_restriction plugin YAML using Jinja template and policy2plugin.json mapping.
_ Modular functions, clear comments, mandatory/optional fields.
_ Usage: python apigee_policy2ip_restriction.py <policy.xml> <output.yaml>
"""
import sys
import os
import json
...
def parse_accesscontrol_policy(xml_path):
    tree = ET.parse(xml_path)
    root = tree.getroot()
    config = {'allow': [], 'deny': []}
    for allow in root.findall('Allow'):  # Example: <Allow>1.2.3.4</Allow>
        if allow.text:
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python apigee_policy2ip_restriction.py <policy.xml> <output.yaml>")
        sys.exit(1)
    xml_path = sys.argv[1]
    ...
    config = parse_accesscontrol_policy(xml_path)
