"""
Enterprise_grade script to convert Apigee VerifyAPIKey/KeyValueMapOperations/BasicAuthentication policy to Kong key_auth plugin YAML using Jinja template and policy2plugin.json mapping.
_ Modular functions, clear comments, mandatory/optional fields.
_ Usage: python apigee_policy2key_auth.py <policy.xml> <service_name> <output.yaml>
"""
import sys
import os
import json
...
def parse_key_auth_policy(xml_path):
    """
    Parse Apigee key_auth related policy XML and extract config for key_auth plugin.
    Args:
...
def main():
    ...
    """
    if len(sys.argv) != 4:
        print("Usage: python apigee_policy2key_auth.py <policy.xml> <service_name> <output.yaml>")
        sys.exit(1)
    xml_path = sys.argv[1]
    ...
    kong_plugin = mapper.get(apigee_policy_type, 'key_auth')
