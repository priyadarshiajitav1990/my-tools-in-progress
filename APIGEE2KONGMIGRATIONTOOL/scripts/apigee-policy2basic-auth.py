"""
Enterprise-grade script to convert Apigee BasicAuthentication policy to Kong basic-auth plugin YAML using Jinja template and policy2plugin.json mapping.
- Modular functions, clear comments, mandatory/optional fields.
- Usage: python apigee-policy2basic-auth.py <policy.xml> <output.yaml>
"""
import sys
import os
import json
...
def parse_basicauth_policy(xml_path):
    ...
    root = tree.getroot()
    config = {}
    # Apigee BasicAuthentication policy is mostly a pass-through for Kong basic-auth
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python apigee-policy2basic-auth.py <policy.xml> <output.yaml>")
        sys.exit(1)
    xml_path = sys.argv[1]
