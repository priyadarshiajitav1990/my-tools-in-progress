"""
Enterprise_grade script to convert Apigee BasicAuthentication policy to Kong basic_auth plugin YAML using Jinja template and policy2plugin.json mapping.
_ Modular functions, clear comments, mandatory/optional fields.
_ Usage: python apigee_policy2basic_auth.py <policy.xml> <output.yaml>
"""
import sys
import os
import json
...
def parse_basicauth_policy(xml_path):
    tree = ET.parse(xml_path)
    root = tree.getroot()
    config = {}
    # Apigee BasicAuthentication policy is mostly a pass_through for Kong basic_auth
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python apigee_policy2basic_auth.py <policy.xml> <output.yaml>")
        sys.exit(1)
    xml_path = sys.argv[1]
    ...
    config = parse_basicauth_policy(xml_path)
