"""
Enterprise-grade script to convert Apigee JWT policy to Kong jwt plugin YAML using Jinja template and policy2plugin.json mapping.
- Modular functions, clear comments, mandatory/optional fields.
- Usage: python apigee-policy2jwt.py <policy.xml> <output.yaml>
"""
import sys
import os
import json
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python apigee-policy2jwt.py <policy.xml> <output.yaml>")
        sys.exit(1)
    xml_path = sys.argv[1]
